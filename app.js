...
function drawActivity(data){
  const width=820,height=410,margin={top:40,right:25,bottom:55,left:130},svg=chartFrame("#activity-chart",width,height);
...
function drawEngagement(data){
  const width=820,height=480,margin={top:28,right:30,bottom:65,left:78},svg=chartFrame("#engagement-chart",width,height);
  ...
  const points=svg.append("g").selectAll("circle").data(data).join("circle").attr("class","scatter-point").attr("cx",d=>x(d.post_count)).attr("cy",d=>y(d.avg_reddit_score)).attr("r",d=>d.month_number===2?7:5).attr("fill",d=>colors.get(d.subreddit)).attr("stroke",d=>d.month_number===2?"#171714":"#fff").attr("stroke-width",d=>d.month_number===2?2:1);
...
function drawSentiment(data,summary){
  const width=820,height=475,margin={top:25,right:125,bottom:55,left:78},svg=chartFrame("#sentiment-chart",width,height);
